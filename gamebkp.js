...
funcoesTroca(
    figurasTabela(
        newGame(figs, linesGroup, columnsGroup)
    ), linesGroup, columnsGroup
);
...
function funcoesTroca(table, linesGroup, columnsGroup) {
    ...
    for (i = 0; i < figOptions.length; i++) {
        figOptions[i].addEventListener("click", function() {
            if (elementSelected != null) {
                ...
                let nome = this.getAttribute('src').replace(/.\/img\//g, "");
                nome = nome.replace(/.svg/g, "");
                table[elementSelected.getAttribute('row')][elementSelected.getAttribute('col')] = nome;
                //console.log('test', elementSelected, elementSelected.getAttribute('row'), elementSelected.getAttribute('col'));
                verificaRepeticoes(table, linesGroup, columnsGroup, [elementSelected.getAttribute('row'), elementSelected.getAttribute('col')]);
                console.log(table);
            }
        });
    }
...
function figurasTabela(tabela) {
    ...
    for (let linha = 0; linha < tabela.length; linha++) {
        for (let coluna = 0; coluna < tabela[linha].length; coluna++) {
            ...
            } else { tabela[linha][coluna] = 'vazio' }
            contador++;
        }
    }
    return tabela;
...
function verificaRepeticoes(table, linesGroup, columnsGroup, posicao) {
    //Mesmo grupo
    let grupos = posicoesGrupos(table, linesGroup, columnsGroup).possibilidades;
    let grupo = [];
...
function posicaoGrupo(grupos, posicao) {
    for (let i = 0; i < grupos.length; i++) {
        for (let j = 0; j < grupos[i].length; j++) {
            if (grupos[i][j][0] == posicao[0] && grupos[i][j][1] == posicao[1]) return i;
        }
    }
}